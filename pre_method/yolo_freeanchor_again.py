...
class free_anchor(nn.Module):
    stride = None  # strides computed during build
    export = False  # onnx export

    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        super(free_anchor, self).__init__()
        self.nc = nc  # number of classes
        self.no = 2  # number of outputs per anchor
        # self.nl = len(anchors)
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.m = nn.ModuleList(nn.Conv2d(ch[i], self.no * self.na[i], 1) for i in range(len(ch)-1))  # output conv

        # self.m0 = nn.Conv2d(ch[0], self.no * 1, 1)
        # self.m1 = nn.Conv2d(ch[1], self.no * self.na, 1)
        # self.m2 = nn.Conv2d(ch[2], self.no * self.na, 1)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            bs, _, ny, nx = x[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)
            # print(self.na[i])
            x[i] = x[i].view(bs, self.na[i], self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            ...
            if not self.training:  # inference
                ...
                k = x[3][i].sigmoid()
                k[..., 0:2] = (k[..., 0:2] * 2) ** 3
                y[:,0,:,:, 2:4] = y[:,0,:,:, 2:4]*k[...,0:2]* self.stride[i]*2
                # if i !=0:
                #     continue
                    # pass
                # else:
...
class Model(nn.Module):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b = mi.bias.view(m.na[i], -1)  # conv.bias(255) to (3,85)
            # i+=1
