    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # loss, loss_items = compute_loss(pred, targets.to(device),model,gt_agument=True)  # loss scaled by batch_size
                loss, loss_items = compute_loss(pred, targets.to(device), model,opt.gt_agument)  # loss scaled by batch_size
                if torch.isnan(loss):
                    continue
                if rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='free_yolo/free-yolov3.yaml', help='model.yaml path')
    parser.add_argument('--gt_agument', action='store_true', help='agument GT while compute loss')
